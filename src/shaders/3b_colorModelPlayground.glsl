...
uniform float iTime;
uniform float iGammaPre;
uniform float iGammaPost;
...
uniform float iToneMappingExposure;
...
void applyToneMapping(inout vec3 col) {
    // Ziel beim Tone Mapping
    // -- Beliebige Wertebereiche [0, +inf] (HDR) nach [0, 1] transformieren
    // -- Kurve muss nur monoton steigend sein. Rechnerisch komplex wäre aber blöd.

    // Beispiel: "ACES / Filmic" Tone Mapping
    vec3 c1 = col * iToneMappingExposure;
    vec3 c2 = c1 * c1;
    vec3 mapped = (2.51 * c2 + 0.03 * c1) / (2.43 * c2 + 0.59 * c1 + 0.14);
...
void applyEffects(inout vec3 col, vec2 uv, bool decodeSRGB) {
    /// Den SRGB-Teil könnt ihr erstmal so hinnehmen
    if (decodeSRGB) {
        col = srgb2linear(col);
    }
...
void main() {
    ...
    /// st: Texture Coordinates in [0, 1]
    /// != uv: Normalized Device Coordinates
    ...
    if (abs(uv.x) < pixelSize) {
        ...
        applyEffects(fragColor.rgb, uv, true);
    }
}
